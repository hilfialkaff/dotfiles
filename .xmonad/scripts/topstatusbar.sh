...
QUOTEPATH="/home/$USER/.xmonad/files/quotes"
SCRIPTPATH="/home/$USER/.xmonad/scripts/"
INTERVAL=1
...
checkMail() {
    num_mail="N/A"
    if [ $conn != "None" ]; then
        num_mail=$(python $SCRIPTPATH/count_mail.py >&1)
...
printBar | dzen2 -xs 1 -x $X_POS -y $Y_POS -w $WIDTH -h $HEIGHT -fn $FONT -ta 'l' -bg $DZEN_BG -fg $DZEN_FG -p -e ''&
printBar | dzen2 -xs 2 -x $X_POS -y $Y_POS -w $WIDTH -h $HEIGHT -fn $FONT -ta 'l' -bg $DZEN_BG -fg $DZEN_FG -p -e ''&
